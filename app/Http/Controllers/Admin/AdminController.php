...
class AdminController extends Controller
{
    ...
    {
        // Fetch the first or only contract
        $contract = Contract::first();

        // If no contract exists, create one with default content
        if (!$contract) {
            $contract = Contract::create(['content' => '']);
        }

        return view('backend.hr-contract.contract', compact('contract'));
    }
    ...
    public function update(Request $request, Contract $contract)
    {
        ...
        return redirect()->route('contracts.edit')->with('success', 'Contract updated successfully.');
    }
    /**
