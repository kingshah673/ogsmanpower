...
class AgencyListService
{
    ...
    {
        if (auth('sanctum')->check() && auth('sanctum')->user()->role == 'agency') {
            return $this->respondError(__('unauthorized_access'));
        }

        $query = Agency::with('user', 'user.contactInfo')->withCount([
        ...
        if ($request->has('lat') && $request->has('long') && $request->lat != null && $request->long != null) {
            $ids = $this->agency_location_filter($request->lat, $request->long);
    ...
    public function agency_location_filter($latitude, $longitude)
    {
        $distance = 50;
        ...
        $data = Agency::select('id')->selectRaw("$haversine AS distance")
