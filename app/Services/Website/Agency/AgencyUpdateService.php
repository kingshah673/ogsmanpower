...
use App\Notifications\Website\Agency\EditApproveNotification;
use Carbon\Carbon;
...
class AgencyUpdateService
{
    ...
    public function execute($request, $job): Job
    {
        ...
        if (setting('edited_job_auto_approved') || $job->status == 'pending') {
            ...
            $edited_exist = Job::where('parent_job_id', $job->id)->where('agency_id', auth()->user()->agency->id)->first();
            if ($edited_exist) {
                // Job title update
                ...
                $main_job = Job::create([
                    ...
                    'education_limit' => $request->education_limit,
                    'experience_limit' => $request->experience_limit,
        ...
        if ($request->has('dynamic_inputs')) {
            foreach ($request->dynamic_inputs as $input) {
                if (isset($input['id']) && isset($input['value'])) {
                    ...
                    AgencyAttributeTranslation::updateOrCreate(
                        [
                            'agency_id' => currentAgency()->id, // Match the current Agency
                            'job_id' => $job->id, // Match the current Agency

                            'Agency_attribute_id' => $input['id'], // Match the Agency attribute
                        ],
                        [
        ...
        if (isset($request->agencyQuestions) && $request->has('agencyQuestions')) {
            $job->questions()->sync($request->get('agencyQuestions'));
        }
