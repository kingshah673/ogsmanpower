                        ...
                        <thead>
                            <tr>
                                ...
                                @if (auth()->user()->hasRole('superadmin'))
                                    ...
                                    @endif
                                    <th width="10%">{{ __('Featured Candidate') }}</th>
                                    <th width="10%">{{ __('Resume Subscription') }}</th>
                                                ...
                                                <a href="javascript:void(0)" class="active-status">
                                                    <label class="switch ">
                                                        <input data-id="{{ $candidate->id }}" type="checkbox"
                                                            class="success is_candidate_featured-switch change-active-status"
                                                            {{ $candidate->is_candidate_featured == 1 ? 'checked' : '' }}>
                                                        <span class="slider round"></span>
                                                    </label>
                                                    <p class="{{ $candidate->is_candidate_featured == 1 ? 'active' : '' }}"
                                                        id="is_candidate_featured_{{ $candidate->id }}">
                                                        {{ $candidate->is_candidate_featured == 1 ? __('Featured') : __('Non-featured') }}
                                                    </p>
                                                </a>
                                            </td>
                                            <td tabindex="0">
                                                <a href="{{ route('candidate.resume.subscription', $candidate->id) }}"
...
@section('script')
    ...
    <script>
        ...
        $('.is_candidate_featured-switch').on('change', function() {
            var is_candidate_featured = $(this).prop('checked') == true ? 1 : 0;
            var id = $(this).data('id');
            $.ajax({
                type: "GET",
                dataType: "json",
                url: '{{ route('candidate.is_featured.change') }}',
                data: {
                    'is_candidate_featured': is_candidate_featured,
                    'id': id
                },
                success: function(response) {
                    toastr.success(response.message, 'Success');
            ...
            if (is_candidate_featured == 1) {
                $(`#is_candidate_featured_${id}`).text("{{ __('Featured') }}")
            } else {
                $(`#is_candidate_featured_${id}`).text("{{ __('Non-Featured') }}")
            }
        });
    </script>
    <script>
