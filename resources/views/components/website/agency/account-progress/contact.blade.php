...
<form action="{{ route('agency.profile.complete', auth()->user()->id) }}" method="post">
    @method('PUT')
    @csrf
    ...
    <fieldset>
                ...
                @if (config('templatecookie.map_show'))
                    ...
                    <div class="card-footer row mt-4 border-0">
                        ...
                        <div class="location_secion">
                            {{ __('country') }}: <span
                                class="location_country">{{ $agency_country ?: $session_country }}</span> <br>
                    ...
                    <div class="card-body pt-0 row">
                        <div class="col-12">
                            <x-website.location.country-state-city-select
                                ...
                                :selected-country="old('country', session('selectedCountryId', $user->agency->country))"
                                :selected-state="old('state', session('selectedStateId', $user->agency->region))"
                                :selected-city="old('district', session('selectedCityId', $user->agency->district))"
                            />
                            @error('location')
                                <span class="ml-3 text-md text-danger">{{ $message }}</span>
                            @enderror
            ...
            <div class="dashboard-account-setting-item">
                ...
                <div class="row">
                    <div class="col-lg-6 mb-3">
                        ...
                        <input class="phonecode @error('phone') is-invalid border-danger @enderror"
                            name="phone" type="text" value="{{ old('phone', $user->contactInfo?->phone) }}"
                        ...
                        <div class="fromGroup has-icon2">
                            <div class="form-control-icon">
                                <input class="form-control @error('email') is-invalid @enderror"
                                   name="email" type="text" placeholder="{{ __('email_address') }}"
        ...
        @unless($longForm)
        <a href="{{ url('agency/account-progress?social') }}">
