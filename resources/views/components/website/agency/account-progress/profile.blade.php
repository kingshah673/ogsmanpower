                ...
                <div class="row">
                    <div class="col-lg-6 mb-3">
                        ...
                        <select name="organization_type_id"
                            class="rt-selectactive  @error('organization_type_id') is-invalid @enderror"
                            id="organization_type_id">
                            @foreach ($organizationTypes as $type)
    <option
        {{ $type->id == old('organization_type_id', optional($user->agency)->organization_type_id) ? 'selected' : '' }}
        value="{{ $type->id }}">
        
        {{ optional($type->translations->first())->name }}
...
@endforeach
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <select type="text"
                            class="rt-selectactive @error('industry_type_id') is-invalid @enderror text-uppercase"
                            name="industry_type_id" id="industry_type">
                            @foreach ($industryTypes as $type)
                                <option
                                    {{ $type->id == old('industry_type_id', $user->agency->industry_type_id) ? 'selected' : '' }}
                                    value="{{ $type->id }}">
                                    {{ $type->name }}
                                </option>
                            @endforeach
                    ...
                    <div class="col-lg-6 mb-3">
                        ...
                        <select type="text" name="team_size_id"
                            ...
                            @foreach ($teamSizes as $size)
                                <option
                                    {{ $size->id == old('team_size_id', $user->agency->team_size_id) ? 'selected' : '' }}
                                    value="{{ $size->id }}">
                                    {{ $size->name }}
                                </option>
                            @endforeach
                        ...
                        <div class="fromGroup has-icon2">
                            <div class="form-control-icon">
                                <input name="website" class="form-control @error('website') is-invalid @enderror"
                                    ...
                                    value="{{ old('website', $user->agency->website) }}">
                    ...
                    <div class="col-12">
                        ...
                        <textarea id="default" name="vision" class="ckeditor @error('vision') is-invalid @enderror"
                            placeholder="{{ __('agency_vision') }}">{{ old('vision', $user->agency->vision) }}</textarea>
                        @error('vision')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ __($message) }}</strong>
                            </span>
        ...
        @unless($longForm)
        <a href="{{ url('agency/account-progress') }}">
