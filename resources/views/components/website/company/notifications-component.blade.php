@php
    $unreadCount = companyUnreadNotifications();
    $notifications = companyNotifications();
    $employerFallbackUrl = route('company.allNotification');
@endphp
<li class="cw-notif-item">
    <div class="notification-icon cw-notif-bell" title="{{ __('notifications') }}" role="button" tabindex="0" aria-label="{{ __('notifications') }}">
        ...
        <div class="notification-bar cw-notif-panel" onclick="event.stopPropagation()">
                    ...
                    <ul>
                        @foreach ($notifications as $noti)
                            @php
                                ...
                                $url = $data['url'] ?? $data['url2'] ?? $employerFallbackUrl;
                            @endphp
            ...
            <a href="{{ route('company.allNotification') }}" class="cw-notif-footer">{{ __('view_all_notifications') }}</a>
        </div>
    </div>
</li>
