    ...
    <div class="cw-apps-header">
        <div>
            ...
            <div class="cw-apps-meta">
                <span>Manage candidates invited to interview. Accept, reject, reschedule, or mark complete — each action notifies the candidate.</span>
            </div>
        </div>
        <div>
            <a href="{{ route('agency.applications') }}" class="btn btn-outline-secondary btn-sm">All Applications</a>
        </div>
    </div>

    <div class="cw-apps-tabs">
        <a class="cw-apps-tab {{ $activeOutcome === 'all' ? 'is-active' : '' }}" href="{{ route('agency.interviews', ['outcome' => 'all', 'q' => request('q')]) }}">
            All <span class="count">{{ $counts['all'] ?? 0 }}</span>
        </a>
        <a class="cw-apps-tab {{ $activeOutcome === 'scheduled' ? 'is-active' : '' }}" href="{{ route('agency.interviews', ['outcome' => 'scheduled', 'q' => request('q')]) }}">
            Scheduled <span class="count">{{ $counts['scheduled'] ?? 0 }}</span>
        </a>
        <a class="cw-apps-tab {{ $activeOutcome === 'rescheduled' ? 'is-active' : '' }}" href="{{ route('agency.interviews', ['outcome' => 'rescheduled', 'q' => request('q')]) }}">
            Rescheduled <span class="count">{{ $counts['rescheduled'] ?? 0 }}</span>
        </a>
        <a class="cw-apps-tab {{ $activeOutcome === 'completed' ? 'is-active' : '' }}" href="{{ route('agency.interviews', ['outcome' => 'completed', 'q' => request('q')]) }}">
            Completed <span class="count">{{ $counts['completed'] ?? 0 }}</span>
        </a>
    </div>

    <form method="GET" action="{{ route('agency.interviews') }}" class="cw-apps-toolbar mb-3">
    ...
    @forelse($interviews as $app)
        @php
            ...
            $detailUrl = ($candidate && $app->job_id)
                ? route('agency.application.detail', [$candidate->id, $app->job_id])
                : '#';
        @endphp
        <div class="cw-iv-card">
            ...
            <div class="cw-iv-actions">
                <form method="POST" action="{{ route('agency.update.interview') }}">
                    @csrf
            ...
            <form method="POST" action="{{ route('agency.update.interview') }}" class="cw-iv-date-row">
                @csrf
                <input type="hidden" name="id" value="{{ $app->id }}">
                ...
                <button type="submit" class="btn btn-warning btn-sm">Reschedule &amp; Notify</button>
            </form>
        </div>
    @empty
        <div class="glass-card"><div class="glass-card-body text-center py-5">
            <h4>No interviews yet</h4>
            <p class="text-muted mb-0">Move a candidate to <strong>Interview</strong> status from <a href="{{ route('agency.applications') }}">Applications</a>.</p>
        </div></div>
    @endempty
