...
<div class="container-fluid px-3">
    <div class="cw-apps-header">
        <div>
            ...
            <div class="cw-apps-meta">
                <span>{{ $counts['all'] ?? 0 }} total applications across your jobs</span>
            </div>
        </div>
        <div>
            <a href="{{ route('company.myjob') }}" class="btn btn-outline-secondary btn-sm">View Jobs</a>
        </div>
    </div>

    <div class="cw-apps-tabs">
        <a class="cw-apps-tab {{ $activeStatus === 'all' ? 'is-active' : '' }}"
           href="{{ route('company.applicants', array_merge($baseParams, ['status' => 'all'])) }}">
            Applicants <span class="count">{{ $counts['all'] ?? 0 }}</span>
        </a>
        <a class="cw-apps-tab tab-shortlisted {{ $activeStatus === 'shortlisted' ? 'is-active' : '' }}"
           href="{{ route('company.applicants', array_merge($baseParams, ['status' => 'shortlisted'])) }}">
            Shortlisted <span class="count">{{ $counts['shortlisted'] ?? 0 }}</span>
        </a>
        <a class="cw-apps-tab {{ $activeStatus === 'interview' ? 'is-active' : '' }}"
           href="{{ route('company.applicants', array_merge($baseParams, ['status' => 'interview'])) }}"
    ...
    <div class="cw-apps">
        <aside class="cw-apps-filters">
            ...
            <form method="GET" action="{{ route('company.applicants') }}">
                <input type="hidden" name="status" value="{{ $activeStatus }}">

                <div class="cw-apps-filter-group" style="border-top:0;padding-top:0;">
                    <label class="cw-apps-filter-label">Job</label>
                    <select name="job_id" class="form-select">
                        <option value="">All jobs</option>
                        @foreach($jobs as $j)
                            <option value="{{ $j->id }}" @selected((string)request('job_id')===(string)$j->id)>{{ $j->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="cw-apps-filter-group">
                    ...
                    <input type="text" name="q" class="form-control" value="{{ request('q') }}" placeholder="Name">
                </div>
                ...
                <a class="btn-reset" href="{{ route('company.applicants') }}">Reset</a>
            </form>
        </aside>

        <div class="cw-apps-main">
            <form method="GET" action="{{ route('company.applicants') }}" class="cw-apps-toolbar">
                <input type="hidden" name="status" value="{{ $activeStatus }}">
                @foreach(request()->except(['q','sort','page','status']) as $k => $v)
                    @if(is_scalar($v))
                        <input type="hidden" name="{{ $k }}" value="{{ $v }}">
                    @endif
                @endforeach
                <input type="text" name="q" class="form-control" value="{{ request('q') }}" placeholder="Search applicants by name">
            ...
            @forelse($applications as $app)
                @include('frontend.pages.company.partials.applicant-card', ['app' => $app, 'showJob' => true])
            @empty
                <div class="cw-apps-empty">No applicants yet. When seekers apply to your jobs, they will appear here.</div>
            @endforelse
